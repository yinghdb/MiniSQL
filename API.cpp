...
void API::deleteWithWhere(){
	...
	for (recordLeftId = 0; recordLeftId < recordLeftCount; recordLeftId++){
		...
		if (if_show){
			...
			char headChar;
			char charTemp[256];
			for (int attrId = 0; attrId < totalCount; attrId++){
				ki.attribute = attrList[attrId].attributeName;
				ki.indexNum = recordLeft[recordLeftId];
				ki.value.type = attrList[attrId].type;
				headChar = *(char*)(recordGetChar + p2);
				p2 += 1;
				switch (ki.value.type){
				case 'i':
					if (attrList[attrId].has_index && headChar != 'N'){
						intTemp = *(int*)(recordGetChar + p2);
						ki.value.intKey = intTemp;
						ki.value.length = sizeof(int);
						index.Delete(catalog.DB_Used, tableName, &ki);
					}
					p2 += sizeof(int);
					break;
				case 'f':
					if (attrList[attrId].has_index && headChar != 'N'){
						floatTemp = *(float*)(recordGetChar + p2);
						ki.value.floatKey = floatTemp;
						ki.value.length = sizeof(float);
						index.Delete(catalog.DB_Used, tableName, &ki);
					}
					p2 += sizeof(float);
					break;
				case 'c':
					if (attrList[attrId].has_index && headChar != 'N'){
						for (int i = 0; i < attrList[attrId].type_count; i++){
							charTemp[i] = *(char*)(recordGetChar + p2 + i);
						}
						charTemp[attrList[attrId].type_count] = '\0';
						ki.value.stringKey = charTemp;
						ki.value.length = attrList[attrId].type_count;
						index.Delete(catalog.DB_Used, tableName, &ki);
					}
					p2 += attrList[attrId].type_count + 1;
					break;
				}
			}
			*/
